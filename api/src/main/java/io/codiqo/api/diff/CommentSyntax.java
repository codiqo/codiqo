...
/**
 * Comment grammar for a single trimmed line: an optional line-comment prefix, the block-comment delimiters, and the
 * optional block-continuation marker (the leading {@code *} of a Java block-comment body). One marker scan serves
 * every C-style language and XML markup — only the markers differ.
 *
 * <p>A unified-diff fragment is never a parseable document, so an AST/markup parser cannot run here; full-file
 * comment detection lives in the language modules, e.g. {@code JavaLineCountAnalyzer} via PMD.
 */
...
public class CommentSyntax {
    ...
    // line-comment only, no block form: properties files, TOML, YAML, shell
    public static final CommentSyntax HASH = new CommentSyntax("#", null, null, null);
