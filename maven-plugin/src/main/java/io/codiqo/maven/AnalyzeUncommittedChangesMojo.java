 ...
 * failed; {@code codiqo.submit=false} keeps the run entirely local. It is the one goal a developer runs before anything is committed, so it is also where the browser
...
public class AnalyzeUncommittedChangesMojo extends AbstractAnalyzeMojo {
    ...
    protected void doLlmScoring(SubmissionContext ctx) throws Exception {
        if (submit) {
             ...
             * must fail too — a script or IDE task reading only the exit code would otherwise treat it as a clean run.
             * a poll timeout stays a success: the analysis is still running server-side and awaitCompletion already
             * logged where to read the result.
             */
