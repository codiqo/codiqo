...
class FileAnalysisPopulator implements SubmissionPopulator {
    ...
    private CodeUnitModel createCodeUnitModel(SubmissionContext ctx, FileAnalysis fileAnalysis,
                                               AffectedSymbolInfo affectedSymbol, JavaCodeBlockInfo javaBlock,
                                               LocationModel locationModel, Path workTree) {
    ...
    private void populateLsp4jSymbolInfo(SubmissionContext ctx, Lsp4jAffectedSymbolInfo lsp4jSymbol,
                                          CodeUnitModel codeUnitModel, JavaInfoModel infoModel, Path workTree) {
    ...
    private void populateCoverage(JavaCodeBlockInfo javaBlock, CodeUnitModel codeUnitModel) {
    ...
    private void populateMethodCalls(JavaCodeBlockInfo javaBlock, JavaInfoModel infoModel) {
    ...
    private void populateDiagnostics(JavaCodeBlockInfo javaBlock, CodeUnitModel codeUnitModel) {
    ...
    private void populateTypeInfo(JavaCodeBlockInfo javaBlock, CodeUnitModel codeUnitModel, JavaInfoModel infoModel) {
    ...
    private void populateMetrics(SubmissionContext ctx, FileAnalysis fileAnalysis,
                                  JavaCodeBlockInfo javaBlock, CodeUnitModel codeUnitModel) {
        javaBlock.metrics().subscribe(metrics -> {
            ...
            // Track AFFECTED (commit-level) metrics
            fileAnalysis.project().ifPresent(spec -> {
    ...
    private void populateAffectedCoverage(SubmissionContext ctx, FileAnalysis fileAnalysis, JavaCodeBlockInfo javaBlock) {
        javaBlock.coverage().subscribe(cov -> {
            if (cov.hasCoverageData()) {
                fileAnalysis.project().ifPresent(spec -> {
                    ModuleQualityTracker tracker = ctx.getQualityTrackers().getUnchecked(spec.getId());
