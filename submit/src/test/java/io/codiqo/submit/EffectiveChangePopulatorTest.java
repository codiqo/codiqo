...
class EffectiveChangePopulatorTest {
    ...
    void promotesWhollyAddedMethodToNew() {
        CodeUnitModel unit = modifyMethod(20, 23, List.of());
        SubmissionContext ctx = contextWith(javaFile(NEW_METHOD_DIFF, unit));

        new EffectiveChangePopulator().accept(ctx);

        assertEquals(OperationEnum.NEW, unit.getOperation());
        assertEquals(null, unit.getEffectiveLinesChanged());
    }
    @Test
    void keepsGenuinelyModifiedMethodAsModify() {
        CodeUnitModel unit = modifyMethod(9, 14, List.of());
        SubmissionContext ctx = contextWith(javaFile(JAVA_DIFF, unit));

        new EffectiveChangePopulator().accept(ctx);

        assertEquals(OperationEnum.MODIFY, unit.getOperation());
    }
    @Test
    void keepsFullyRewrittenMethodAsModify() {
        ...
        // every line in the span is added (isWhollyAdded), but the old body shows as in-range deletions,
        // so it is a rewrite, not new code — must stay MODIFY.
        assertEquals(OperationEnum.MODIFY, unit.getOperation());
    }
    @Test
